...
dati %>%
  ...
  summarise(
    count = n(),
    median = median(NO, na.rm = TRUE),   #### FC il nome della variabile da analizzare
...
#### Se serve modificare l'ordine di comparsa dei Trattamenti/Gruppi/Genotipi
dati_8$Genotype <- ordered(dati_8$Genotype, levels = c("WT", "Tg"))


### my_comparisons <- list(c("Veh", "LPS"), c("LPS", "LPS/"))
...
my_comparisons1 <- list(c("CTRL", "LPS"), c("LPS", "LPS/KT172"))
...
### Per limitare il numero di confronti...
#my_comparisons2 <- list(c("CTRL", "LPS", "LPS/KT172"))
...
pwc <- dati %>% 
  ...
  t_test(NO ~ Genotype,
         comparisons = NULL) %>%
  adjust_pvalue(method = "sidak") %>%
  add_significance("p.adj")
pwc

pwc <- dati %>% 
  group_by(Treatment) %>%
  emmeans_test(NO ~ Genotype,
               comparisons = NULL,
               p.adjust.method = "sidak"        #### Alternatives can be "none", "fdr", "hochberg", "hommel", "holm", "tukey", "mvt", "dunnettx", "sidak", "scheffe", "bonferroni". For details, see stats::p.adjust() and emmeans::summary().
  ) 
pwc
...
a2 <- aov(NO ~ Genotype*Treatment, data=dati)
summary(a2) 
EMM = emmeans(a2, ~ Genotype*Treatment);EMM
cld(EMM, adjust = "sidak")
...
p <- ggboxplot(
  ...
  add.params = list(dotsize = 0.8, alpha = 1)
)+
