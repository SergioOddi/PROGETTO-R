#####Analisi protein APP, App_alpha, App_beta
##29 Febbraio 2024
...
stat.test <- dati %>%
  ...
  adjust_pvalue(method = "none") %>% ##### Qui si decidono i confronti: bonferroni, holm, etc.
  add_significance()
...
colori_customizzati <- c("URB" = "#4DBBD5B2", "Veh" = "#E64B35B2")
...
p <- ggboxplot(
  ...
  add="dotplot",
  add.params = list(dotsize = 0.6,      #### Dimensione dei punti
                    alpha = 1, 
  ...
  labs(title="",                                   #se serve il titolo
       x = "Treatment",                                  #nome dell'asse delle ascisse
       y = "Protein expression (ratio to β-actin)") +      #nome dell'asse delle ordinate
  ...
  theme(text = element_text(size=sizeT, color = "black", face="bold", family = "Helvetica"),           #qui size stabilisce caratteristiche del nome delle variabili nel riquadro in alto ai grafici
        axis.text.x = element_text(size=14, angle = 1, hjust = 0.5, face="bold", color="black"),  #decide dimensione e inclinazione del nome dei gruppi sulle ascisse
  ...
  facet_wrap(~var, ncol = 5, scales = "free_y") # Modifica qui per disporre le faccette su una singola riga con scale y libere
...
p + stat_pvalue_manual(stat.test,
                       ...
                       bracket.nudge.y = 0.1,  ### distanza tra l'ultimo valore e la parentesi...
                       bracket.shorten = 0,  ####???
                       color = "red",
                       linetype = 1,       ####
                       step.increase = 0, ####
                       tip.length = 0.03, #### DIMENSIONE DELLA stanghetta della PARENTESI DI SIGNIFICATIVITA
                       hide.ns = F,   ####### Nasconde i confronti ns
