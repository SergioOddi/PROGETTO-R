...
dati <- Isoprostanoidi_MCI_e_sani_PEA_Placebo_To_T1 # dataset classico, con ogni variabile per colonna

####### Eliminare/selezionare gruppi
### Se serve rimuovere osservazioni falsate
#dati <- subset(dati, Group != "AD SEVERE")
dati <- subset(dati, Group == "MCI_PEA_T0" | Group == "MCI_PEA_T1")
...
dati <- dati[, c(1, 3, 7:10)] #Prendo solo la colonna gruppi e le colonne con le variabili
...
dati <- Isoprostanoidi_MCI_e_sani_PEA_Placebo_Foglio1_1_ # dataset classico, con ogni variabile per colonna
...
#####   Seleziono il vettore Group e le variabili
dati <- dati[, c(4, 8:15)] #Prendo solo la colonna gruppi e le colonne con le variabili
