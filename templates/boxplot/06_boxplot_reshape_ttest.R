###### BOXPLOT MULTIPLI con reshaping per t test
### Versione 2.0: Confronto tra due gruppi (t test)
### 20 marzo 2023
...
#dati <- dati %>% rename("Group" = "GROUP") ### NB: il primo termine dell'eq. sosituisce il secondo
...
p <- ggboxplot(
  ...
  color = "black",
  order =c("CTRL", "PEA"),  # per ordinare i fattori
  ...
  geom_signif(test = "t.test", 
              test.args = list(var.equal = TRUE, paired = FALSE, p.adjust.method = "fdr"),
              comparisons = list(c("CTRL", "PEA")), 
              map_signif_level=TRUE,
              #map_signif_level = function(p) sprintf("p = %.4g", p),  #####   2g: due cifre significative
              #map_signif_level = function(p.adjust) sprintf("p = %.4g", p.adjust),  #####   2g: due cifre significative
              #map_signif_level = function(p) sprintf("p = %.4g", p),  #####   2g: due cifre significative
               step_increase = 1,
#             label = signif,
#              label = "p.signif", 
              hide.ns = TRUE ,           ### se posto vero, non mostra "ns"
              size = 0.2,             #### DIMENSIONE DELLA PARENTESI DI SIGNIFICATIVITA
              ...
              # y_position = 0.3,
              # manual=TRUE,
              textsize = 2) +         ###### Dimensione asterisco o NS
...
p + scale_y_continuous(expand = expansion(mult = c(0.1, 0.2)))   ### Visualizza il boxplot  
