...
curva <- nls(E7F13_2 ~ A*exp((-2*tau)/time)*(BesselI((2*tau)/time, 0)+BesselI((2*tau)/time, 1)) + B, data=dati, ##### A, tau e #B sono, risp., il primo, #il secondo, e il terzo coefficiente del modello scelto 
...
plot(E7F13_2 ~ time, data = dati, ylim =c(0.2, 1), #### intervallo delle ordinate
...
names(dati) <- c("time", "1", "2", "3", "4", "5", "6", "7", "8")

###Seleziono le variabili che mi interessano
selezionate <- c("1", "3", "4", "6", "7", "8")
selezionate 
